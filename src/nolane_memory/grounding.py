...
class MemoryRuntime(_BaseMemoryRuntime):
    """v0.6.3 runtime plus opt-in G3 structured-grounding conformance hardening.
    ...
    This surface strengthens enforcement of the existing action/tool grounding contract.
    It does not claim that the runtime can discover every material dependency on its own.
    """
    ...
    def ground_consequence_atom(
        ...
        *,
        ...
        object_matches = [fragment for fragment in frame.fragments if fragment.role_id == role_id]
    ...
    def _close_consequence_groundings(
        ...
        *,
        ...
        for grounding in by_path.values():
            ...
            # Conformance-enforcement step: host-bound provenance joins the consequence
            # read-set, and its currentness is checked before a fence can be minted.
    ...
    def issue_use_fence(
        ...
        *,
        ...
        # MemoryUseFence remains the sole use-time owner. Its persisted dependency set
        # carries the joined forward+grounded read-set, so post-issuance mutation is still
        # caught by the existing consume-time revalidation path.
        return super().issue_use_fence(
            closed_frame,
            principal=principal,
            sink=sink,
