...
class StructuredGroundingConformanceTests(unittest.TestCase):
    """Defense-in-depth enforcement for structured memory-derived consequence atoms.

    The ungrounded control intentionally models a host that failed to carry a material
    prior-memory dependency into its active consequence read-set. rc4 already requires
    action/tool grounding dependencies semantically; this suite verifies that the new
    opt-in structured surface can make that host obligation machine-checkable for bound
    JSON atoms. It is not evidence for a new v0.7 semantic theorem.
    """
    ...
    def _earlier_recipient_frame(self):
        return self.rt.compile_recall(
            "d", "alice", [RecallRole("recipient", self.region_b, "RECIPIENT")], 20
        )

    def _current_action_frame(self):
        return self.rt.compile_recall(
            "d", "alice", [RecallRole("action", self.region_a, "ACTION")], 20
        )
    ...
    def test_legacy_fence_preserves_rc4_behavior_when_host_omits_grounding(self):
        ...
        # Backward compatibility is deliberate: callers that do not opt into structured
        # grounding retain the exact rc4 fence behavior. This is a nonconforming-host
        # negative control, not a valid semantic counterexample to the rc4 specification.
    ...
    def test_structured_grounding_blocks_stale_retained_memory_dependency(self):
        earlier_b = self._earlier_recipient_frame()
        stale_recipient = earlier_b.fragments[0].payload["recipient"]
    ...
    def test_unrelated_mutation_does_not_invalidate_grounded_fence(self):
        ...
        grounding = self._ground_recipient(self._earlier_recipient_frame())
        current_a = self._current_action_frame()
    ...
    def test_model_proposed_grounding_cannot_self_upgrade_to_strong_use(self):
        grounding = replace(
            self._ground_recipient(self._earlier_recipient_frame()),
